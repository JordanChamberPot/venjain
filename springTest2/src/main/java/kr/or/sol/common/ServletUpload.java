   ...
   public static Map<String, Object> uploadEx(HttpServletRequest req, HttpServletResponse res) {
      ...
      try {
         ...
         while (iter.hasNext()) {
            ...
            String name = null;
            String value = null;
            // 파일인지 여부 확인 : isFormFile() -> type=file 이외에 폼 데이터 인지 확인
            if (item.isFormField()) { // 텍스트 입력인 경우
               ...
               // writer, subject, content, passwd, email,
               if (name.equals("num"))
               ...
               if (item.getString("utf-8") != null && !item.getString("utf-8").equals("")) {
                  ...
                  // 저장하고자 하는 파일의 이름
                  newfileName = fileName.substring(0, ext) + "_" + now
                        + fileName.substring(ext, fileName.length());
                  // 데이터 저장 File(위치, 파일명)
